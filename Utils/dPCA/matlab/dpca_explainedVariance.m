...
S = diag(S);
S = S(1:size(W,2));
explVar.cumulativePCA = cumsum(S.^2'/ explVar.totalVar * 100); % in percent of total variance

% dPCA explained variance
Z = W'*X;
for i=1:size(W,2)
    explVar.cumulativeDPCA(i) = 100 - sum(sum((X - V(:,1:i)*Z(1:i,:)).^2)) / explVar.totalVar * 100; % 100 * R^2, or (1-residual variance fraction).
    explVar.componentVar(i) = 100 - sum(sum((X - V(:,i)*Z(i,:)).^2)) / explVar.totalVar * 100; % components are non-orthogonal, so this ultimately will not sum up nicely to give the cumulative variance explained
   
    for j=1:length(Xmargs)
        ZZ = Xmargs{j} - V(:,i)*(W(:,i)'*Xmargs{j});
        explVar.margVar(j,i) = (explVar.totalMarginalizedVar(j) - sum(ZZ(:).^2)) / explVar.totalVar * 100; % the element of this corresponding to the whichMarg of this factor is the component of the objective function that was actually maximized
    end
end
