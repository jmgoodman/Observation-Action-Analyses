...
for monkeyName = monkeyNames
    ...
    hc = [];
    cmat = [];
    for areaName = areaNames
        areaName_ = areaName{1};
        temp = histc(data.(monkeyName_).(areaName_),-1:.1:1);
        hc = horzcat(hc,temp(:)); %#ok<AGROW>
        
        whichColor = strcmpi(colorStruct.labels,areaName_);
        thisColor  = colorStruct.colors(whichColor,:);
        cmat = vertcat(cmat,thisColor); %#ok<AGROW>
    end
    
    figure,b=bar(-.95:.1:.95,hc(1:(end-1),:),1,'stacked');
    for i = 1:3
        set(b(i),'facecolor',cmat(i,:));
    end
    ...
    for legent = 1:numel(legendNames)
        legendNames{legent} = [legendNames{legent},': N=',num2str(numel(data.(monkeyName_).(legendNames{legent})))];
    end
    ...
    print([monkeyName_,'_','stacked','.svg'],'-dsvg')
end
...
%                 'facecolor',thisColor,'facealpha',alph,'edgecolor',[0 0 0]);
...
%                 'facecolor',thisColor,'facealpha',0.2,'edgecolor',[0 0 0]);
%             set(gca,'fontsize',16)
%             set(gcf,'paperposition',[0 0 6 6])
%             xlabel('Passive-Active Index')
%             ylabel('Fraction of units')
