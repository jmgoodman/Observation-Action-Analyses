...
%% load data
sesh_strs = {'Moe46','Moe50','Zara64','Zara68','Zara70'};
% sesh_strs = {'Zara64','Zara68','Zara70'};
% sesh_strs = {'Zara68','Zara70','Moe50'}; % after fixing the PCA # of axes bug, these are sessions that remain to be rerun

% Zara65 is only for visualizing kins during observation and doesn't need to be processed here
...
for sesh_ind = 1:numel(sesh_strs)
    ...
    copts.withMGG.active = makeclassifyopts('transform','none','alignment',{'lighton','move','hold'},'targetcontexts',{'active','control'},'PCAdims',30,'transformdims',20); % no passive for now... maybe you want that though? to verify that cross-classification is built on visual representations??? NO. write first, THEN decide if you need it. You've been stuck in the analysis tweaking step of this loop for far too long as it stands...
    ...
    %%%%%%
    %% Implementing kinematic clustering (todo: run kinematic clustering on the HUMAN kinematics from Zara's data! And assume a similar distribution for Moe's mirror data...)
    if ismoe
        kca = kcstruct_moe;
