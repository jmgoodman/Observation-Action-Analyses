function [Cnoise,Cnoiseunscaled] = dpca_getNoiseCovariance(Xfull, Xtrial, numOfTrials, varargin)
...
if ~options.simultaneous
    ...
    if strcmp(options.type, 'pooled')
        ...
        Cnoiseunscaled = SSnoise;
        Cnoise = SSnoise/size(Xnoise,2);
        dims = size(Xfull);
        dimProd = prod(dims(2:end)); % oh i get it, this ensures we divide by trial counts and not by something substantially stupider.
