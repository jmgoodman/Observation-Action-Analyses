function Batch_extractfeatures_noStatusBar
% Batch_extractfeatures_noStatusBar.m
...
% Update by James Goodman 2020 Feb 19
% Primarily to remove any and all instances of "statusbar" to permit
% calling this function outside the context of the KinemaTracks GUI. Also
% removed any instances of "gui" or its associated handle(s). ALSO removed
% any mention of "lastfile", as the data-saving aspect of this script was
% deleted with the previous update (we just save to a global "kinematics"
% variable instead).
...
% global lastfile; % can be empty [the name of the file with the kinematic data (clunkily loaded by starting the "player")]


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% conspicuously absent: anything dealing with "calibdata".
% it seems to be created & used DURING the recording to convert magnetic
% field perturbations into sensible cartesian coordinate data
% but once it's been used DURING the recording, it seems to serve as little
% more than a "sanity check" for the recordings themselves, as those
% recordings are ALREADY scaled relative to the "calibration" data.
% the calibration data may also offer a "sanity check" for parameters such
% as segment lengths. they may even offer a much more accurate
% determination thereof, IF, of course, one can interpret those calibration
% data. (it seems like time x digit x XYZ coordinate, with each entry
% indicating, in mm, the value of that cartesian coordinate specifying that
% sensor's location, with sensors beling placed on the fingernails, if you
% recall...) (wait, if this is the case... wouldn't you need the reference
% marker? unless... these calibration data ALREADY subtract out the
% reference marker! I think I recall seeing code to that effect in
% calibratehand2 and calibratestatic, although I should double-check that.)
%
% yeah, from eyeballing the "recording" field, the first 3 columns
% (slices?) of each sample seem to correspond to cartesian coordinates, and
% they roughly correspond with the dimensions spanned by the hand in the
% calibdata field when one assumes that the ONLY thing calibdata contains
% is cartesian coordinates (I have to assume the other columns pertain to
% sensor orientations & such). In other words, the hand spans roughly 30 x
% 20 x 30 mm in both calibdata & in the samples of recording, suggtesting
% that these two datasets have already been "calibrated" to the same
% consistent units (presumably mm) and scaling.
%
% might it be possible to use calibdata to create the "static" session for
% OpenSim model scaling? Probably! Although the lack of 1-to-1
% correspondence in the columns & rows would make it a clunky process...
% but the fact I've come this far means I'm not exactly averse to clunky
% processes, am I???
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
