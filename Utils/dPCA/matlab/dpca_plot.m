...
%
%  'Cnoise'
%                           - the conditional noise covariance matrix
%
%  'NoPie'
%                           - whether or not to plot the pie chart. default is false (which means DO plot the pie chart).
...
%     end
    ...
    if isempty(options.CustomLegend)
        ...
        legend(options.CustomLegend,'Location','SouthEast');
    end
...
map = tril(a,-1)+triu(b); % lower part of triangle is correlations; upper part, the dot products of the component vectors
% top part tells you about orthogonality (or lack thereof). bottom part tells you about data correlations. both are normally 0 with PCA, but need not necessarily be linked (see, for instance, the unity matrix transformation of the data: all mutually orthogonal basis vectors, but loads of correlations)
% (note, too, that the top part tells you about the orthogonality of the ENCODERS... the vectors of the DECODER matrix can still be quite nearly orthogonal even if the encoder matrix projects them back onto the same set of dimensions! this is because the decoder matrix is projecting it into a specific partition's data space, and not a "universal" data space!)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% one more time to drill it in:
% top = dot products
% bottom = correlations
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% pie chart
if ~isempty(options.explainedVar) && ~options.NoPie
    ...
    mColors = options.marginalizationColours;
    if ~isempty(options.Cnoise)
        newvec = [options.explainedVar.totalMarginalizedVar(:);trace(options.Cnoise)];
        d      = newvec ./ sum(newvec) * 100;
        
        options.marginalizationNames = vertcat(options.marginalizationNames(:),{'Noise'});
        mColors = vertcat(mColors,[1 1 1]);
    else
        % pass
    end
    ...
    pp = pie(d, ones(size(d)), margNamesPerc);
    
    colorind = 1;
    for ppind = 1:2:numel(pp)
        pp(ppind).FaceColor = mColors(colorind,:);
        colorind = colorind + 1;
    end
    
    %     caxis([1 length(mColors) + 256])
end
