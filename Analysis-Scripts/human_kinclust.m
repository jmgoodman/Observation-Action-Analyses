function clusterstruct = human_kinclust()
%% cleanup
...
for ii = 1:numel(kinsessions)
    ...
    kincat = vertcat( kincat, ...
        squeeze( datastruct.cellform{1}{6}.KinematicData(51,:,passiveinds) )' );
    
    % grab object IDs
    objcat = vertcat( objcat, ...
        triallabels.objects.names(passiveinds) );
    
    seshcat = vertcat( seshcat, ...
        repmat(kinsessions(ii),numel(triallabels.objects.names(passiveinds)),1) );
end
...
save('humanClusters.mat','clusterstruct','-v7.3');
