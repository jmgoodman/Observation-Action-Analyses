function Kin2OpenSim_vJames(kinfile,savefile)
...
%   kinfile     ... specifies full path containing the kinemaitcs or
%   kinematics_i struct of interest. Can also be empty (default). If empty,
%   a GUI will guide selection of the target file.
...
%
%EXAMPLE
%   Kin2OpenSim_vJames('MyDir/MyFile.mat','MySecondDir/MySecondFile.mat')
%
%AUTHOR
%   by Stefan Schaffelhofer
%   adapted and updated by James Goodman, 2020 January 10, DPZ
...
if ~isempty(kinfile)
    assert(ischar(kinfile) || isempty(kinfile),'kinfile must either be an empty array or be a valid string specifying a .mat file location!')
    load(kinfile);
else
    [file,path] = uigetfile({'*KIN.mat';'*KINi.mat';'*.mat'},'Find and select the target .mat file with your kinematics in it. It should end with _KIN.mat or _KINi.mat.');
